package fuzs.enderzoology.common.client.renderer.entity;

import com.mojang.blaze3d.vertex.PoseStack;
import fuzs.enderzoology.common.EnderZoology;
import fuzs.enderzoology.common.client.model.geom.ModModelLayers;
import fuzs.enderzoology.common.client.renderer.entity.layers.EnderminyEyesLayer;
import fuzs.enderzoology.common.world.entity.monster.Enderminy;
import net.minecraft.client.model.monster.enderman.EndermanModel;
import net.minecraft.client.renderer.entity.EntityRendererProvider;
import net.minecraft.client.renderer.entity.MobRenderer;
import net.minecraft.client.renderer.entity.state.EndermanRenderState;
import net.minecraft.resources.Identifier;

public class EnderminyRenderer extends MobRenderer<Enderminy, EndermanRenderState, EndermanModel<EndermanRenderState>> {
    private static final Identifier TEXTURE_LOCATION = EnderZoology.id("textures/entity/enderminy/enderminy.png");

    public EnderminyRenderer(EntityRendererProvider.Context context) {
        super(context, new EndermanModel<>(context.bakeLayer(ModModelLayers.ENDERMINY)), 0.25F);
        this.addLayer(new EnderminyEyesLayer(this));
    }

    @Override
    public EndermanRenderState createRenderState() {
        return new EndermanRenderState();
    }

    @Override
    protected void scale(EndermanRenderState endermanRenderState, PoseStack poseStack) {
        poseStack.scale(0.5F, 0.25F, 0.5F);
    }

    @Override
    public Identifier getTextureLocation(EndermanRenderState endermanRenderState) {
        return TEXTURE_LOCATION;
    }
}
